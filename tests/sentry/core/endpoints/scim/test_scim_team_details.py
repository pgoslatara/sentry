...
class SCIMPrivilegeManagementTest(SCIMTestCase):
    """
    Tests for managing user privileges via special SCIM Groups.

    When members are added to groups named "staff" or "superuser" in the default
    SaaS organization, their is_staff/is_superuser flags should be updated.
    """
    ...
    def setUp(self) -> None:
        ...
        # Create users with organization members
    ...
    def test_adding_to_staff_group_grants_is_staff(self) -> None:
        """Adding a member to the 'staff' group should grant is_staff privilege"""
        with override_settings(
            ...
            assert not user_service.get_user(user_id=self.user_one.id).is_staff
            ...
            assert user_service.get_user(user_id=self.user_one.id).is_staff

    def test_adding_to_superuser_group_grants_is_superuser(self) -> None:
        """Adding a member to the 'superuser' group should grant is_superuser privilege"""
        with override_settings(
            SENTRY_MODE=SentryMode.SAAS, SENTRY_DEFAULT_ORGANIZATION_ID=self.organization.id
        ):
            ...
            assert not user_service.get_user(user_id=self.user_two.id).is_superuser
            ...
            assert user_service.get_user(user_id=self.user_two.id).is_superuser

    def test_removing_from_staff_group_revokes_is_staff(self) -> None:
        """Removing a member from the 'staff' group should revoke is_staff privilege"""
        with override_settings(
            SENTRY_MODE=SentryMode.SAAS, SENTRY_DEFAULT_ORGANIZATION_ID=self.organization.id
        ):
            staff_team = self.create_team(
            ...
            # First add the member and grant privilege
            ...
            # Verify is_staff was revoked
            assert not user_service.get_user(user_id=self.user_one.id).is_staff

    def test_regular_team_does_not_affect_privileges(self) -> None:
        """Adding to a regular team (not staff/superuser) should not change privileges"""
        with override_settings(
            SENTRY_MODE=SentryMode.SAAS, SENTRY_DEFAULT_ORGANIZATION_ID=self.organization.id
        ):
            ...
            assert not user_service.get_user(user_id=self.user_one.id).is_staff
            assert not user_service.get_user(user_id=self.user_one.id).is_superuser
            ...
            # Verify privileges were not changed
            assert not user_service.get_user(user_id=self.user_one.id).is_staff
            assert not user_service.get_user(user_id=self.user_one.id).is_superuser

    def test_non_default_org_does_not_grant_privileges(self) -> None:
        """Privileges should only be granted in the default organization"""
        with override_settings(
            SENTRY_MODE=SentryMode.SAAS, SENTRY_DEFAULT_ORGANIZATION_ID=self.organization.id
        ):
            # Create a different organization (not default org)
            other_org = self.create_organization(name="Other Org")
            other_member = self.create_member(user=self.user_one, organization=other_org)

            # Set up SCIM for the other organization
            ...
            # Login as the SCIM user for the other org
            ...
            # Verify is_staff was NOT granted (because not default org)
            assert not user_service.get_user(user_id=self.user_one.id).is_staff

    def test_non_saas_mode_does_not_grant_privileges(self) -> None:
        """Privileges should only be granted in SaaS mode"""
        with override_settings(
            ...
            # Verify is_staff was NOT granted (because not SaaS mode)
            assert not user_service.get_user(user_id=self.user_one.id).is_staff
